...
export async function getSummaryById(id: string) {
    try {
        ...
        const [summary] = await sql `SELECT id, user_id, summary_text, original_file_url, title, created_at, updated_at, status, file_name, LENGTH(summary_text) - LENGTH(REPLACE(summary_text, ' ', '')) + 1 as word_count from pdf_summaries where id= ${id}`;
